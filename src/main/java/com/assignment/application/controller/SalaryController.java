...
public class SalaryController {
    ...
    public ResponseEntity<SalaryDTO> addSalaryInfo(@PathVariable(StringConstant.COMPANY_ID) @NonNull Long companyId,
        ...
        String userId = verifyUsers.authorizeUser(token, companyId + "/" + employeeId + StringConstant.SALARY, StringConstant.POST);
    ...
    public ResponseEntity<List<SalaryDTO>> getSalaryInfo(@PathVariable(StringConstant.COMPANY_ID) @NonNull Long companyId,
                                                         @PathVariable("emp_id") @NonNull String employeeId,
                                                         @RequestHeader(StringConstant.ACCESS_TOKEN) String token,
                                                         Pageable pageable) {
    ...
    public ResponseEntity<String> updateSalaryOfEmployee(@PathVariable(StringConstant.COMPANY_ID) @NonNull Long companyId,
                                                         @PathVariable("emp_id") @NonNull String employeeId,
                                                         @RequestBody @Valid SalaryEmployeeUpdate salaryEmployeeUpdate,
                                                         @RequestHeader(StringConstant.ACCESS_TOKEN) String token) {
        String userId = verifyUsers.authorizeUser(token, companyId + "/" + employeeId + "/update-salary", StringConstant.PATCH);
